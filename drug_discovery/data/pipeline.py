...
def validate_batch(smiles_list):
    ...
    return {
        ...
        "valid": len(valid),
        "invalid": len(invalid),
        "valid_ratio": len(valid) / max(len(smiles_list), 1),
        "unique_canonical": len(uc),
        "duplicate_count": len(canonical) - len(uc),
        "invalid_smiles": invalid[:20],
    }
...
def lipinski_filter(desc):
    checks = {
        "mw_le_500": desc.get("mol_weight", 0) <= 500,
        "logp_le_5": desc.get("logp", 0) <= 5,
        "hbd_le_5": desc.get("hbd", 0) <= 5,
        "hba_le_10": desc.get("hba", 0) <= 10,
    }
...
def tanimoto_similarity(fp1, fp2):
    ...
    union = np.sum(fp1) + np.sum(fp2) - inter
