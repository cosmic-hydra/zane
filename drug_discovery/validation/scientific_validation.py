...
def spearman_rho(y_true, y_pred):
    from scipy.stats import spearmanr; rho, _ = spearmanr(y_true, y_pred)
...
def enrichment_factor(y_true, y_score, fraction=0.01):
    n = len(y_true); top_k = max(1, int(n * fraction))
...
def bemis_murcko_scaffold(smiles):
    try:
        from rdkit import Chem; from rdkit.Chem.Scaffolds import MurckoScaffold
        mol = Chem.MolFromSmiles(smiles)
        return MurckoScaffold.MurckoScaffoldSmiles(mol=mol, includeChirality=False) if mol else smiles
    except ImportError: return smiles

def scaffold_split(smiles_list, frac_train=0.8, frac_val=0.1, seed=42):
    scaffolds = defaultdict(list)
    for i, s in enumerate(smiles_list): scaffolds[bemis_murcko_scaffold(s)].append(i)
    sets = sorted(scaffolds.values(), key=len, reverse=True)
    rng = random.Random(seed); rng.shuffle(sets)
    n = len(smiles_list); tc = int(n*frac_train); vc = int(n*(frac_train+frac_val))
    train, val, test = [], [], []
    for ss in sets:
        if len(train)+len(ss) <= tc: train.extend(ss)
        elif len(train)+len(val)+len(ss) <= vc: val.extend(ss)
        else: test.extend(ss)
...
def scaffold_kfold(smiles_list, n_folds=5, seed=42):
    scaffolds = defaultdict(list)
    for i, s in enumerate(smiles_list): scaffolds[bemis_murcko_scaffold(s)].append(i)
    sets = list(scaffolds.values()); rng = random.Random(seed); rng.shuffle(sets)
    folds = [[] for _ in range(n_folds)]; sizes = [0]*n_folds
    for ss in sorted(sets, key=len, reverse=True):
        smallest = min(range(n_folds), key=lambda i: sizes[i])
        folds[smallest].extend(ss); sizes[smallest] += len(ss)
    return [(sum([folds[j] for j in range(n_folds) if j!=i], []), folds[i]) for i in range(n_folds)]

# --- Statistical tests ---
def paired_ttest(a, b):
    from scipy.stats import ttest_rel; s, p = ttest_rel(a, b)
    return {"t_statistic": float(s), "p_value": float(p), "significant_005": p<0.05}
def wilcoxon_test(a, b):
    from scipy.stats import wilcoxon
    try: s, p = wilcoxon(a, b); return {"statistic": float(s), "p_value": float(p), "significant_005": p<0.05}
    except ValueError: return {"statistic": 0.0, "p_value": 1.0, "significant_005": False}
def bootstrap_ci(values, n_bootstrap=10000, ci=0.95, seed=42):
    ...
    return {"mean": float(values.mean()), "std": float(values.std()),
            "ci_lower": float(np.percentile(boot, 100*alpha)), "ci_upper": float(np.percentile(boot, 100*(1-alpha)))}

# --- Experiment Report ---
@dataclass
class ExperimentReport:
    experiment_id: str = ""; config_hash: str = ""; seed: int = 42
    model_name: str = ""; dataset: str = ""; split_method: str = "scaffold"
    n_folds: int = 5; fold_metrics: List[Dict[str, float]] = field(default_factory=list)
    ...
    timestamp: str = ""; duration_seconds: float = 0.0

    def compute_aggregates(self):
        if not self.fold_metrics: return
    ...
    def save(self, path):
        ...
        with open(path, "w") as f: json.dump(self.to_dict(), f, indent=2, default=str)
