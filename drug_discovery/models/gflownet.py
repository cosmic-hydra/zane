...
class GFlowNetConfig:
    hidden_dim: int = 256; num_layers: int = 4; max_atoms: int = 38
    atom_vocab: int = 10; bond_types: int = 4; temperature: float = 1.0
    reward_exponent: float = 2.0; learning_rate: float = 5e-4
...
class GFlowNetPolicy(nn.Module):
    ...
    def forward(self, state):
        ...
        return {"atom_logits": self.add_atom_head(g)/self.config.temperature, "stop_logit": self.stop_head(g), "graph_embedding": g}

class GFlowNetBackwardPolicy(nn.Module):
    def __init__(self, config): super().__init__(); h=config.hidden_dim; self.net=nn.Sequential(nn.Linear(h,h),nn.SiLU(),nn.Linear(h,1))
...
class GFlowNetTrainer:
    ...
    def __init__(self, config, reward_fn=None, device="cpu"):
        ...
        self.optimizer=torch.optim.Adam(list(self.forward_policy.parameters())+list(self.backward_policy.parameters())+[self.log_Z], lr=config.learning_rate)
    ...
    def _sample_trajectory(self):
        ...
        log_pf=torch.tensor(0.,device=self.device); log_pb=torch.tensor(0.,device=self.device)
        for step in range(self.config.max_atoms):
            ...
            if step>0 and torch.rand(1,device=self.device)<sp: log_pf=log_pf+torch.log(sp+1e-8); break
            if step>0: log_pf=log_pf+torch.log(1-sp+1e-8)
            probs=F.softmax(out["atom_logits"],-1); atom=torch.multinomial(probs,1)
            log_pf=log_pf+torch.log(probs[atom]+1e-8); atoms=torch.cat([atoms,atom.view(-1)])
            if atoms.size(0)>1:
                ni=atoms.size(0)-1; edges=torch.cat([edges,torch.tensor([[ni,ni-1],[ni-1,ni]],device=self.device)],1)
    ...
    def sample(self, n=10):
        self.forward_policy.eval(); results=[]
        for _ in range(n): t,_,_=self._sample_trajectory(); results.append({"atoms":t["atoms"].cpu().numpy(),"num_atoms":t["num_atoms"]})
        self.forward_policy.train(); return results
