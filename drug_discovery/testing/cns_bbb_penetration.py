...
class BioavailabilityScreener:
    ...
    def calculate_cns_mpo(self, smiles: str) -> float:
        ...
        def f_clogp(x):
            return 1.0 if x <= 3.0 else np.exp(-((x - 3.0) ** 2) / 2.0)

        def f_mw(x):
            return 1.0 if x <= 360.0 else np.exp(-((x - 360.0) ** 2) / 5000.0)
        ...
        def f_tpsa(x):
            if 40 <= x <= 90:
                ...
                return np.exp(-((x - 90.0) ** 2) / 1000.0)

        def f_hbd(x):
            return 1.0 if x <= 0 else 0.8 if x == 1 else 0.0

        def f_pka(x):
            return np.exp(-((x - 8.0) ** 2) / 10.0)
    ...
    def predict_p_glycoprotein_efflux(self, smiles: str) -> bool:
        ...
        return bool(mw > 400 and (logp > 3.0 or hba > 8))
