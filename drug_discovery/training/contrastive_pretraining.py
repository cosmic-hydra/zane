...
class ContrastiveConfig:
    hidden_dim: int = 256; proj_dim: int = 128; temperature: float = 0.1
    augment_node_drop: float = 0.1; augment_edge_drop: float = 0.1
    augment_coord_noise: float = 0.05; learning_rate: float = 1e-3

class GraphAugmentor:
    """Stochastic graph augmentations for contrastive views."""
    def __init__(self, config): self.config = config
    def augment(self, h, pos, edge_index):
        h = self._node_drop(h); edge_index = self._edge_drop(edge_index); pos = self._noise(pos)
    ...
    def _edge_drop(self, ei):
        ...
        return ei
    def _noise(self, pos):
        return pos + torch.randn_like(pos)*self.config.augment_coord_noise if self.config.augment_coord_noise > 0 else pos
...
class PretrainEncoder(nn.Module):
    ...
    def __init__(self, hidden_dim, num_layers=4, max_atomic=100):
        ...
        self.layers = nn.ModuleList([nn.ModuleDict({
            ...
            "ln":nn.LayerNorm(hidden_dim)}) for _ in range(num_layers)])
...
class ContrastivePretrainer(nn.Module):
    ...
    def __init__(self, config: ContrastiveConfig):
        ...
        self.projector = nn.Sequential(nn.Linear(config.hidden_dim,config.hidden_dim),nn.ReLU(),nn.Linear(config.hidden_dim,config.proj_dim))
    ...
    def _encode(self, h, pos, edge_index):
        ...
        for layer in self.encoder.layers:
            ...
            h = layer["ln"](h + layer["upd"](torch.cat([h,agg],-1)))
        return h.mean(0)
    def get_pretrained_encoder(self): return self.encoder
