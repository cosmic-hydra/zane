...
BRENK_SMARTS = {
    ...
    "aldehyde": "[CX3H1](=O)", "epoxide": "C1OC1",
    "thiol": "[SX2H]", "acyl_halide": "[CX3](=[OX1])[FX1,ClX1,BrX1,IX1]",
    "sulfonate_ester": "[SX4](=O)(=O)[OX2]",
    "peroxide": "[OX2][OX2]", "azide": "[NX1]=[NX2]=[NX1]",
    "beta_lactam": "C1C(=O)NC1", "vinyl_halide": "[CX3]=[CX3][FX1,ClX1,BrX1]",
    "aliphatic_nitro": "[CX4][NX3+](=O)[O-]",
    "thiocarbonyl": "[CX3]=[SX1]", "acyl_cyanide": "[CX3](=[OX1])C#N",
    "phosphoramide": "[PX4](=O)([NX3])([NX3])",
    "polycyclic_aromatic": "c1ccc2c(c1)ccc1ccc3ccccc3c1c2",
}
REACTIVE_METAB_SMARTS = {
    "aniline": "[NH2]c1ccccc1", "nitroaromatic": "[cR1][NX3+](=O)[O-]",
    "thiophene": "c1ccsc1", "furan": "c1ccoc1",
    "hydroquinone": "[OH]c1ccc([OH])cc1", "terminal_alkyne": "C#[CH]",
    "hydroxamic_acid": "[CX3](=O)[NX3][OH]", "epoxide_precursor": "C=C[CX4]",
}

@dataclass
class AlertResult:
    alert_name: str; alert_class: str; pattern: str; severity: str
@dataclass
class StructuralAlertReport:
    smiles: str; total_alerts: int = 0
    ...
    is_clean: bool = True; risk_score: float = 0.0
    def to_dict(self):
        return {"smiles":self.smiles,"total":self.total_alerts,"clean":self.is_clean,
                "risk":round(self.risk_score,3),"pains":[a.alert_name for a in self.pains_alerts],
                "brenk":[a.alert_name for a in self.brenk_alerts],"reactive":[a.alert_name for a in self.reactive_alerts]}
...
def _check_smarts(smiles, smarts_dict, cls, severity):
    ...
    try:
        ...
        if mol is None: return alerts
        for name, pat in smarts_dict.items():
            try:
                q = Chem.MolFromSmarts(pat)
                if q and mol.HasSubstructMatch(q): alerts.append(AlertResult(name, cls, pat, severity))
            except: continue
    except ImportError:
        smi = smiles.lower()
        for name, hit in {"nitro":"[n+](=o)[o-]" in smi,"azo":"n=n" in smi,"thiol":"sh" in smi}.items():
            if hit: alerts.append(AlertResult(name, cls, "heuristic", severity))
    return alerts
...
class StructuralAlertScreener:
    ...
    def batch_screen(self, smiles_list):
        ...
        return reports
    def get_clean_molecules(self, reports): return [r.smiles for r in reports if r.is_clean]
